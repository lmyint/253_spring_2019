...
ggplot(grad, aes(x = admit, y = pred_prob1)) + geom_boxplot() + labs(x = "True admission status", y = "Model-predicted probability of admission", title = "Model 1: predictors = GRE")
ggplot(grad, aes(x = admit, y = pred_prob2)) + geom_boxplot() + labs(x = "True admission status", y = "Model-predicted probability of admission", title = "Model 2: predictors = GRE, GPA, undergrad \"prestige\" rank")
```
...
plot(candy_clust, xlab = "Candies", ylab = "")
```
...
@. **A simple linear regression model**    
    ...
    <center>
    `Price` = $\hat{f}$(`Living.Area`) + $\varepsilon$ = $\hat\beta_0$ + $
    hat\beta_1$`Living.Area` + $\varepsilon$
    </center>
...
@. **Interpreting coefficients in a multivariate model**    
    ...
    a. The average price increase in price for each extra square foot
    b. The average price increase in price for each extra square foot, holding constant fuel type
    c. The average price increase in price for each extra square foot, only for houses that use fuel type 2
    d. The average price increase in price for each extra square foot, only for houses that use fuel type 3
    e. The average price increase in price for each extra square foot, only for houses that use fuel type 4
