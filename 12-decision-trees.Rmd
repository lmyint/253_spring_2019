...
`r NextQ()`. **Trees can also be used for regression!**    
    For classification, we use average Gini index as a measure of node (im)purity for tree building. Using *residuals*, come up with a measure of node (im)purity that can be used for regression tree building. At the same time, you'll also need to come up with a way to make a prediction for a leaf node.
